...
def initial_setup_task():
    ...
    if missing_vars:
        print(
            f"[Hello Flow - WARNING] Missing environment variables: {missing_vars}. Some tools might not work."
        )
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        planner = Planner(
            use_llm=False
        )  # Set use_llm=True and ensure OPENAI_API_KEY is set to test LLM planning
        ...
        flow_file_path = deploy(
            graph, flows_dir="./projects/aegis_orchestrator_mvp/flows"
        )  # Ensure this path is correct relative to execution
        ...
        print(
            "Ensure you are in the project root or PYTHONPATH includes the 'src' directory."
        )
