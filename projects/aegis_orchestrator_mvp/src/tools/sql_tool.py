...
from src.feedback import (
    record as feedback_record,
)  # Changed from ..feedback and aliased
...
class SQLTool:
    ...
    def invoke(
        ...
        try:
            ...
            if (
                success_status
                and "data" in tool_output
                and isinstance(tool_output["data"], list)
            ):
                if len(tool_output["data"]) > 5:  # Arbitrary limit for verbosity
                    output_for_feedback = {
                        **tool_output,
                        "data": f"{len(tool_output['data'])} rows returned (sample: {tool_output['data'][:2]}...)",
                    }
