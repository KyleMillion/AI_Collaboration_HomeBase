...
class EmailAPI:
    ...
    def __init__(self):
        ...
        self.smtp_port = os.getenv(
            "SMTP_PORT"
        )  # Keep as string, smtplib handles conversion
    ...
    def invoke(
        ...
        **kwargs,
        ...
        if not all([self.smtp_host, self.smtp_port, self.smtp_user, self.smtp_pass]):
            print(
                "[EmailAPI Error] SMTP credentials not fully configured. Check SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS."
            )
            ...
            try:
                feedback_record(
                    ...
                    tool_name="EmailAPI",
                    success=False,
                    output={"error": "SMTP credentials not configured"},
