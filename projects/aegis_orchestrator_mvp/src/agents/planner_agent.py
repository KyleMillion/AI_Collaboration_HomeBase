...
"""
    ...
    def invoke(
        self, prompt: str, pipeline_id: str = "N/A", variant: str = "N/A", **kwargs
    ) -> dict:
        ...
        print(
            f"[PlannerAgent] Invoking LLM with prompt: {enriched_prompt[:100]}..."
        )  # Log snippet

        try:
            resp = self.llm.invoke(
                [self._sys, HumanMessage(content=enriched_prompt)]
            ).content
            ...
            if json_start != -1 and json_end != -1 and json_start < json_end:
                ...
                print(
                    "[PlannerAgent] LLM response was not valid JSON or not found. Falling back."
                )
                ...
                return {
                    ...
                    "tasks": [
                        {
                            "id": "task.echo_raw_llm_response",
                            "agent": "SlackAPI",
                            "params": {
                                "message": f"Planner LLM Fallback: Could not parse plan from LLM for prompt '{prompt}'. Raw LLM response: {resp}"
                            },
                        }
                    ],
                }
            ...
            print(
                f"[PlannerAgent] JSONDecodeError: {e}. Raw response: {resp}. Falling back."
            )
            ...
            return {
                ...
                "tasks": [
                    {
                        "id": "task.echo_json_error",
                        "agent": "SlackAPI",
                        "params": {
                            "message": f"Planner LLM Error: Could not decode JSON from LLM for prompt '{prompt}'. Error: {e}. Raw LLM response: {resp}"
                        },
                    }
                ],
            }
            ...
            return {
                ...
                "tasks": [
                    {
                        "id": "task.echo_unexpected_error",
                        "agent": "SlackAPI",
                        "params": {
                            "message": f"Planner LLM Unexpected Error: {e} for prompt '{prompt}'."
                        },
                    }
                ],
            }
