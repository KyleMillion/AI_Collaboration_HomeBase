...
CANNED_LLM_RESPONSE_JSON = {
    ...
    "tasks": [
        ...
        },
        {
            ...
            "params": {
                "channel": "#new-users",
                "message": "New user onboarded: Test User (test@example.com)",
            },
            "dependencies": ["task_1_create_user_account"],
        },
    ],
...
class TestPlannerAgentLLM(unittest.TestCase):
    ...
    def test_invoke_llm_successful_parse(self, MockChatOpenAI):
        ...
        planner = PlannerAgent(
            model="gpt-test-mock"
        )  # Use a distinct model name for test
        ...
        MockChatOpenAI.assert_called_once_with(
            model_name="gpt-test-mock", temperature=0.1
        )
        ...
        self.assertEqual(
            len(result_graph["tasks"]), len(CANNED_LLM_RESPONSE_JSON["tasks"])
        )
        self.assertEqual(
            result_graph["tasks"][0]["agent"],
            CANNED_LLM_RESPONSE_JSON["tasks"][0]["agent"],
        )
    ...
    def test_invoke_llm_json_decode_error_fallback(self, MockChatOpenAI):
        ...
        MockChatOpenAI.assert_called_once_with(
            model_name="gpt-test-mock-fallback", temperature=0.1
        )
        self.assertIsInstance(result_graph, dict)
        self.assertTrue(
            result_graph["id"].startswith("pipeline.fallback.echo_raw_response_for_")
        )
        self.assertEqual(len(result_graph["tasks"]), 1)
        self.assertEqual(result_graph["tasks"][0]["agent"], "SlackAPI")
        self.assertIn(
            "Planner LLM Fallback", result_graph["tasks"][0]["params"]["message"]
        )
        self.assertIn(
            "This is not JSON.", result_graph["tasks"][0]["params"]["message"]
        )
