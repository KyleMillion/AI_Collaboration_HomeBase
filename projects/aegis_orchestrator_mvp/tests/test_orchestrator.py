...
dummy_prefect_deployments = types.ModuleType(
    "prefect.deployments"
)  # Create dummy deployments submodule
...
dummy_prefect.deployments = (
    dummy_prefect_deployments  # Attach deployments submodule to prefect dummy
)

sys.modules["prefect"] = dummy_prefect
sys.modules["prefect.deployments"] = (
    dummy_prefect_deployments  # Add deployments submodule to sys.modules
)
# ----------------------------------------------------------------------
...
def _simple_graph():
    ...
    return {
        ...
        "tasks": [{"id": "t1", "agent": "MockAgent", "params": {"x": 1}}],
...
def ensure_feedback_db_connection():
    ...
    test_db_file_for_orchestrator_tests = os.path.join(
        data_path, "test_feedback.db"
    )  # Ensure this is the same DB file used by test_feedback
