...
class TestFeedback(unittest.TestCase):
    ...
    def test_record_new_success(self, mock_variant_metric_counter):
        ...
        cur = feedback.con.execute(
            "SELECT success, failure FROM ab_stats WHERE pipeline = 'pipe1' AND variant = 'varA'"
        )
        row = cur.fetchone()
    ...
    def test_record_new_failure(self, mock_variant_metric_counter):
        ...
        cur = feedback.con.execute(
            "SELECT success, failure FROM ab_stats WHERE pipeline = 'pipe1' AND variant = 'varB'"
        )
        row = cur.fetchone()
        self.assertIsNotNone(row)
    ...
    def test_record_multiple_updates(self, mock_variant_metric_counter):
        ...
        cur = feedback.con.execute(
            "SELECT success, failure FROM ab_stats WHERE pipeline = 'pipe2' AND variant = 'varA'"
        )
        row = cur.fetchone()
    ...
    def test_best_variant_no_data(self):
        self.assertEqual(
            feedback.best_variant("pipe_empty", default="default_val"), "default_val"
        )

    def test_best_variant_not_enough_trials(self):
        # 19 trials, should use default
        for _ in range(10):
            feedback.record("pipe_few", "varX", True)
        for _ in range(9):
            feedback.record("pipe_few", "varX", False)
        self.assertEqual(
            feedback.best_variant("pipe_few", default="default_X"), "default_X"
        )
    ...
    def test_best_variant_selects_highest_rate_sufficient_trials(self):
        ...
        for _ in range(20):
            feedback.record("pipe_multi", "varA", True)
        # varB: 25 trials, 80% success (20 S, 5 F)
        for _ in range(20):
            feedback.record("pipe_multi", "varB", True)
        for _ in range(5):
            feedback.record("pipe_multi", "varB", False)
        # varC: 10 trials, 100% success (not enough trials)
        for _ in range(10):
            feedback.record("pipe_multi", "varC", True)
    ...
    def test_best_variant_tie_break_by_trials(self):
        ...
        for _ in range(18):
            feedback.record("pipe_tie", "varA", True)
        for _ in range(2):
            feedback.record("pipe_tie", "varA", False)
        ...
        for _ in range(27):
            feedback.record("pipe_tie", "varB", True)
        for _ in range(3):
            feedback.record("pipe_tie", "varB", False)
    ...
    def test_best_variant_default_if_all_variants_low_trials(self):
        for _ in range(5):
            feedback.record("pipe_all_low", "var1", True)
        for _ in range(10):
            feedback.record("pipe_all_low", "var2", False)
        self.assertEqual(
            feedback.best_variant("pipe_all_low", default="default_low"), "default_low"
        )
